...
/*****************************************************************************
  ...
  ***************************************************************************/
// parses NULL terminated string for valid dotted decimal IP address
// must start with a decimal digit
// must have 4 decimal byte size values separated by '.' chars
// may have 1 trailing space, tab, cr, lf, or '/' char
// only uses passed IP_ADDR* when valid IP is parsed.
// call with NULL IP_ADDR pointer to test for valid IP string 

BOOL StringToIPAddress(char *str, IP_ADDR *buffer)
{
   char c;
   char byteIndex;     // which octet in IP_ADDR
   WORD ans;           // octet value accumulator
   BOOL in_num;        // are we in a number?
   IP_ADDR tmp_ip;     // temporary IP_ADDR holder
   ...
   while(c = *str++)
   {
       c -= '0';   // char < '0' will underflow
    
       if (((int)c <= 9)&&((int)c>=0))  // got a digit
       {
           in_num = TRUE;
           ans = ans * 10 + c;
           if(ans > 255)
           ...
           if(byteIndex < 4)   // store ans so far
           {
               tmp_ip.v[byteIndex] = (BYTE)ans;
           }
       }
       else    // got non-digit
       {
           if(!in_num) {
               return FALSE;   // must follow a digit
           }
           if(byteIndex < 3)   // in first 3 octets
           {
               if(c != '.'-'0'){    // only allowed non-digit is '.'
                    return FALSE;
               }
           }
           else    // check for allowed trailing char
           {
               if((c != ' '-'0')&&(c != '\t'-'0')&&(c != '\r'-'0')&&(c != '\n'-'0')&&(c != '/'-'0'))
                   return FALSE;
           }

           if(byteIndex < 4)   // still in IP part of string
           {
               byteIndex++;
               ans = 0;
           }

           in_num = FALSE;     // got non-digit
       }
   }

   if(c == '\0' && in_num)
       byteIndex++; // for IP with no trailing chars

   if(byteIndex == 4)
   {
       if(buffer != NULL)
           buffer->Val = tmp_ip.Val;
...
BOOL StringToIPAddress_OLD(BYTE* str, IP_ADDR* IPAddress)
{
	DWORD_VAL dwVal;
...
#if !defined(__18CXX) && !defined(HI_TECH_C) && !defined(__C32_VERSION__)
void ultoa(DWORD Value, BYTE* Buffer)
{
	BYTE i;
	DWORD Digit;
