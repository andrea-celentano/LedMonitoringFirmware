 ...
 *    module, all incoming CAN data out of the TCP socket.
 *	
 *********************************************************************
 * FileName:        CAN2TCPBridge.c
 * Dependencies:    TCP, Hardware CAN module, ARP (optional), 
 *					DNS (optional)
 * Processor:       PIC32
...
// Ring buffers for transfering data to and from the CAN ISR:
...
static BYTE vCANRXFIFO[60];
static BYTE vCANTXFIFO[60];
...
void __attribute__((vector(46), interrupt(ipl4), nomips16)) CAN1InterruptHandler(void)
{
	...
	else if((CANGetModuleEvent(CAN1) & CAN_TX_EVENT) != 0)
	{
		if(CANGetPendingEventCode(CAN1) == CAN_CHANNEL0_EVENT)
		{
			...
			// Transmit data, if pending, if possible
			if(TXHeadPtr > TXTailPtr)
			{
				int nMsg;
				int i;
				
				nMsg = TXHeadPtr - TXTailPtr-2;
				if(message != NULL)
			    {
					...
					msgID = ((*TXTailPtr++)<<8);
					msgID = ((msgID) | (*TXTailPtr++));
			        message->msgSID.SID 	= msgID;		
			        message->msgEID.IDE 	= 0;			
			        message->msgEID.DLC 	= nMsg;
					
					for(i = 0; i< nMsg; i++)
					{
			        	message->data[i] = *TXTailPtr++;	
					}
