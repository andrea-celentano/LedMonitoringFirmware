...
void AnnounceIP(void)
{
	UDP_SOCKET	MySocket;
	BYTE 		i;
        static BYTE     IPdigits[4];
	// Open a UDP socket for outbound broadcast transmission
	MySocket = UDPOpen(2860, NULL, ANNOUNCE_PORT); //NULL means "BROADCAST"
	...
	UDPPutROMString((ROM BYTE*)"My IP address: \n");

        for(i=0;i<4;i++)
	{

            uitoa((WORD)AppConfig.MyIPAddr.v[i], IPdigits);
            UDPPutROMString((ROM BYTE*)IPdigits);
            if (i!=3) UDPPut('.');
	}/**/
         UDPPut('\n');
