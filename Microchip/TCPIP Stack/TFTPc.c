...
{
    DEBUG(DBPRINTF("Opening a connection..."));
...
TFTP_RESULT TFTPIsOpened(void)
{
    ...
    default:
        ...
        if ( ARPIsResolved(&MutExVar.group1._hostInfo.IPAddr,&MutExVar.group1._hostInfo.MACAddr) )
        {
...
TFTP_RESULT TFTPIsGetReady(void)
{
    ...
    case SM_TFTP_WAIT_FOR_DATA:
        ...
        if ( bTimeOut == TRUE )
        {
            ...
            {
                DEBUG(DBPRINTF("TFTPIsGetReady(): Timeout.\n"));
            ...
            {
                DEBUG(DBPRINTF("TFTPIsGetReady(): TFTPOpen Retry.\n"));
                return TFTP_RETRY;
            }
            else
            {
                DEBUG(DBPRINTF("TFTPIsGetReady(): ACK Retry #%d...,\n", _tftpRetries));

                // Block number was already incremented in the ACK attempt,
                // so decrement it.
...
TFTP_RESULT TFTPIsPutReady(void)
{
    ...
    case SM_TFTP_WAIT_FOR_ACK:
        ...
        if ( bTimeOut )
        {
            if ( _tftpRetries++ > (TFTP_MAX_RETRIES-1) )
            {
                DEBUG(DBPRINTF("TFTPIsPutReady(): Timeout.\n"));
            ...
            {
                DEBUG(DBPRINTF("TFTPIsPutReady(): Retry.\n"));
                _tftpState = SM_TFTP_WAIT;
